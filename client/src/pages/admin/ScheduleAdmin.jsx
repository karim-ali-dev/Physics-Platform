...
import { Plus, Pencil, Loader2, CalendarDays, Clock, X, Eye, EyeOff, Layers, Hash } from 'lucide-react';
import { api } from '../../api';
...
export default function ScheduleAdmin() {
  ...
  const openAdd = () => {
    setEditing(null);
    setForm({ ...blank, grade: gradeFilter !== 'الكل' ? gradeFilter : GRADES[0] });
    setError('');
    setModal(true);
  };
  ...
  const openEdit = (f) => {
    setEditing(f);
    setForm({ grade: f.grade, day: f.day, start_time: f.start_time, end_time: f.end_time, note: f.note, period: f.period || 'الليل', tag: f.tag || '', tag_active: f.tag_active !== 0, sort_order: f.sort_order, active: f.active !== 0 });
    setError('');
    setModal(true);
  };
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="جدول الحصص الحضورية لكل المراحل — بيتعرض في صفحة «مواعيد الدروس» بالموقع"
        action={
      ...
      {loading ? (
        ...
        <div className="mt-8">
          <Empty text="مفيش مواعيد لحد دلوقتي — اضغط «إضافة موعد جديد» فوق." />
        </div>
      ) : filtered.length === 0 ? (
        <div className="mt-8">
          <Empty text="مفيش مواعيد مطابقة للفلاتر دي." />
        </div>
      ) : (
                    ...
                    <div key={f.id} className={`flex flex-wrap items-center justify-between gap-3 px-5 py-3 ${f.active ? '' : 'opacity-50'}`}>
                      <div className="flex min-w-0 flex-wrap items-center gap-2">
                        ...
                        <span className="flex items-center gap-1 text-sm font-bold text-neon-300">
                          <Clock size={13} /> {f.start_time}{f.end_time ? ` - ${f.end_time}` : ''}
                        </span>
                        {f.tag && (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink-950/80 p-4 backdrop-blur-sm" onClick={() => setModal(false)}>
          <div className="card max-h-[92vh] w-full max-w-xl overflow-y-auto p-6" onClick={(e) => e.stopPropagation()}>
            <div className="mb-6 flex items-center justify-between">
              <h3 className="flex items-center gap-2 text-lg font-extrabold"><CalendarDays size={20} className="text-brand-400" /> {editing ? 'تعديل الموعد' : 'إضافة موعد جديد'}</h3>
            ...
            <form onSubmit={submit} className="space-y-5">
              <div className="grid gap-5 sm:grid-cols-2">
                ...
                <Field label="وقت البداية" required>
                  <TextInput value={form.start_time} onChange={(e) => setForm({ ...form, start_time: e.target.value })} placeholder="مثال: 6:00 م" required maxLength={30} />
                </Field>
                <Field label="وقت النهاية">
                  <TextInput value={form.end_time} onChange={(e) => setForm({ ...form, end_time: e.target.value })} placeholder="مثال: 7:30 م" maxLength={30} />
                </Field>
                <Field label="ملاحظة تظهر جنب المعاد (اختياري)" hint="مثال: فيه امتحان، مراجعة، حصة إضافية">
