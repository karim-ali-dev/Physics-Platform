...
import { fmt12Time, cairoClock } from '../../utils/schedule';
...
const PRIORITY_META = {
  high: { label: 'عاجل', color: 'bg-red-500/15 text-red-300' },
  medium: { label: 'متوسط', color: 'bg-brand-500/15 text-brand-300' },
  low: { label: 'منخفض', color: 'bg-white/10 text-white/55' }
};
const BOARD_ORDER = ['in_progress', 'pending', 'done'];
...
export default function TasksAdmin() {
  ...
  const filtered = useMemo(() => items.filter((t) => {
    const byStatus = statusFilter === 'all' ? true : statusFilter === 'overdue' ? isOverdue(t) : t.status === statusFilter;
    const byPriority = priorityFilter === 'all' || t.priority === priorityFilter;
    const byGrade = gradeFilter === 'all' || t.grade === gradeFilter;
    const byQ = !q.trim() || (t.title + ' ' + t.description + ' ' + t.category).includes(q.trim());
    return byStatus && byPriority && byGrade && byQ;
  }), [items, statusFilter, priorityFilter, gradeFilter, q]);
  ...
  const setStatus = async (t, status) => {
    await api(`/api/admin/tasks/${t.id}/status`, { method: 'PATCH', body: JSON.stringify({ status }) }).catch(() => {});
    load();
  };

  const del = async (id) => {
    await api(`/api/admin/tasks/${id}`, { method: 'DELETE' }).catch(() => {});
    load();
  };
  ...
  const TaskCard = ({ t }) => {
    ...
    return (
      <div className={`group relative rounded-xl border bg-ink-900/70 p-4 transition-all hover:border-brand-500/40 ${overdue ? 'border-red-500/40' : 'border-white/10'}`}>
        ...
        {(t.due_date || t.due_time) && (
          <div className={`mt-3 flex items-center gap-1.5 text-[11px] font-bold ${overdue ? 'text-red-300' : 'text-white/50'}`}>
            <CalendarDays size={12} />
            {t.due_date ? t.due_date.split('-').reverse().join('/') : '—'}
            {t.due_time && <span className="flex items-center gap-1"><Clock size={11} /> {fmt12Time(t.due_time)}</span>}
          </div>
        )}
  ...
  return (
    <div>
      ...
      {/* Filters */}
      <div className="mt-6 rounded-2xl border border-white/10 bg-ink-900/60 p-4">
      ...
      {view === 'board' && (
        ...
        : items.length === 0 ? <div className="mt-8"><Empty text="مفيش مهام لسه — اضغط «إضافة مهمة جديدة» فوق." /></div>
        : (
          <div className="mt-6 grid gap-4 lg:grid-cols-3">
            {BOARD_ORDER.map((st) => {
              const meta = STATUS_META[st];
              const cols = filtered.filter((t) => t.status === st);
              return (
                <div key={st} className={`rounded-2xl border p-3 ${meta.border}`}>
                  ...
                  <div className="space-y-3">
                    {cols.length === 0 && <p className="py-6 text-center text-xs text-white/35">مفيش مهام هنا.</p>}
                  ...
                  <div className="flex shrink-0 items-center gap-3">
                    {(t.due_date || t.due_time) && (
                      <span className={`flex items-center gap-1 text-xs font-bold tabular-nums ${overdue ? 'text-red-300' : 'text-white/50'}`}>
                        <CalendarDays size={13} /> {t.due_date ? t.due_date.split('-').reverse().join('/') : ''} {t.due_time && fmt12Time(t.due_time)}
                      </span>
                    )}
